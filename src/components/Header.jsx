...
import {
  Globe, RefreshCw, Briefcase, ShieldCheck, MapPin, Bell, LogOut,
...
const ProfilePopup = ({ onClose, onLogoutRequest, t }) => {
  const { currentUser, updateUserProfile, changeUserId, addNotification } = useAppContext();
  ...
  const handleChangeId = async () => {
    setIdError('');
    if (!newId.trim()) { setIdError('Please enter a new ID'); return; }
    if (newId.trim() === currentUser.id) { setIdError('Same as current ID'); return; }
    setSaving(true);
    const result = await changeUserId(currentUser.id, newId.trim());
    if (result.success) {
      addNotification('Your ID has been updated successfully.', 'success', newId.trim());
      setChangingId(false);
      setNewId('');
    } else {
      setIdError(result.error || 'Failed to change ID');
    }
    setSaving(false);
  };
          ...
          <div className="p-3 bg-slate-50 rounded-2xl">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-black text-slate-700">{currentUser.id}</span>
                {editing && <button onClick={() => setChangingId(!changingId)} className="text-[8px] font-black text-indigo-500 bg-indigo-50 px-2 py-1 rounded-lg hover:bg-indigo-100">Change</button>}
              </div>
            </div>
            {changingId && (
              <div className="mt-2 space-y-2 animate-in slide-in-from-top-1 duration-150">
                <input value={newId} onChange={e => { setNewId(e.target.value); setIdError(''); }} placeholder="Enter new ID" className="w-full bg-white rounded-xl px-3 py-2 text-sm font-bold text-slate-800 outline-none border border-slate-200"/>
                {idError && <p className="text-[10px] font-bold text-red-500">{idError}</p>}
                <div className="flex gap-2">
                  <button onClick={() => { setChangingId(false); setNewId(''); setIdError(''); }} className="flex-1 py-2 bg-slate-100 text-slate-500 rounded-xl text-[9px] font-black uppercase">Cancel</button>
                  <button onClick={handleChangeId} disabled={saving} className="flex-1 py-2 bg-amber-500 text-white rounded-xl text-[9px] font-black uppercase shadow-sm disabled:opacity-50">{saving ? 'Updating...' : 'Confirm'}</button>
                </div>
                <p className="text-[8px] text-amber-600 font-bold">Warning: This will update your ID across all tasks, messages, groups, and events.</p>
              </div>
            )}
          </div>
...
const InviteModal = ({ onClose, t }) => {
  const { users, addNotification, currentUser } = useAppContext();
  ...
  const createUser = async (e) => {
    e.preventDefault();
    setError('');
    if (adminConfirmId !== currentUser.id) { setError('Admin ID does not match.'); return; }
    if (users.find(u => u.id === form.id)) { setError('Employee ID already exists!'); return; }
    setSending(true);
    ...
    const newUser = { ...form, allowedPages: sanitizedPages, permanent: false, isOnline: false, nickname: form.username };
    await setDoc(doc(db, 'users', form.id), newUser);
    addNotification(`New team member "${form.username}" added.`, 'info', 'all');
    addNotification(`Welcome ${form.username}! You've been added to the system.`, 'success', form.id);
    if (form.email) {
      const result = await sendCredentialsEmail({ username: form.username, id: form.id, email: form.email, role: form.role });
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="bg-white w-full max-w-md rounded-[2.5rem] p-6 shadow-2xl relative animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-2 mb-5">{['add','invite'].map(tab_ => (
          <button key={tab_} onClick={() => setTab(tab_)} className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${tab === tab_ ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-400'}`}>{tab_ === 'add' ? t('addUser') : t('inviteUser')}</button>
        ))}</div>
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-black leading-none flex items-center gap-1.5 min-w-0">
              ...
              {isCEO && <MapPin size={12} className="text-amber-500"/>}
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          ...
          {isCEO && (
            <button
              ...
              className="flex items-center gap-1.5 p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-xl"
              title={t('liveMap')}
            >
              <MapPin size={18}/>
            </button>
          ...
          <div className="relative" ref={mobileMenuRef}>
            ...
            {mobileMenuOpen && (
              <div className="absolute end-0 top-11 w-52 bg-white border border-slate-100 rounded-[1.5rem] shadow-2xl p-2 z-50 animate-in zoom-in-95 duration-150">
                ...
                {isCEO && (
                  <button
                    onClick={() => { navigate('/live-map'); setMobileMenuOpen(false); }}
                    className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-amber-50 text-slate-700"
                  >
                    <MapPin size={16} className="text-amber-600"/>
                    <span className="text-xs font-black uppercase tracking-widest">{t('liveMap')}</span>
                  </button>
                )}
